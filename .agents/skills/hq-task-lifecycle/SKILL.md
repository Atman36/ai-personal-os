...
- `os/05_Control_Plane/operating-policies.json` only when risk, autonomy, approval, or owner authority changes
- relevant `.os_runtime/specs/<task>/LATEST.md` and `.os_runtime/handoffs/<task>/LATEST.md` when the task already exists
...
3. Make the smallest valid change in `os/05_Control_Plane/active-work.json`.
4. If task state changed materially, run `python3 scripts/osctl.py sync`.
5. Run `python3 scripts/osctl.py validate`.
...
- Do not hand-edit `os/02_Planning/Task_Board.md`; render it through `sync`.
- Keep private analysis, raw prospect data, imports, and runtime notes under `.os_runtime/` or outside the repo.
